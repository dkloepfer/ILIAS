...
class gevCourseUtils
{
	...
	public static function getInstance($a_crs_id)
	{
	...
	public static function getInstanceByObj(ilObjCourse $a_crs)
	{
	...
	public static function getInstanceByObjOrId($a_course)
	{
	...
	public static function getLinkTo($a_crs_id)
	{
		return "goto.php?target=crs_".gevObjectUtils::getRefId($a_crs_id)	;
	}

	public static function getCancelLinkTo($a_crs_id, $a_usr_id)
	{
		global $ilCtrl;
	...
	public static function getBookingLinkTo($a_crs_id, $a_usr_id)
	{
	...
	public static function mkDeadlineDate($a_start_date, $a_deadline)
	{
	 ...
	 * the custom id.
	 **/
	public static function createNewCustomId($a_tmplt)
	{
		global $ilDB;
	...
	public static function extractCustomId($a_custom_id)
	{
	 ...
	 * the template.
	 **/
	public static function createNewTemplateCustomId($a_tmplt)
	{
		global $ilDB, $ilLog;
		$gev_settings = gevSettings::getInstance();
		$field_id = $gev_settings->getAMDFieldId(gevSettings::CRS_AMD_CUSTOM_ID);
	 ...
	 */
	public static function getCustomRoles($crs_id)
	{
		global $rbacreview;
	...
	public function insertParticipationStatusAndPoints($changed_by, $user_id, $state, $cpoints)
	{
		if (!is_numeric($state)) {
	...
	public static function formatFee($a_fee)
	{
	...
	protected function moveAccomodationsDurationChanged($a_old_start_date, $a_old_end_date, $a_new_start_date, $a_new_end_date)
	{
		...
		while ($rec = $this->gIldb->fetchAssoc($res)) {
			...
			if ($prearrival && $postdeparture && $amount_of_nights == $old_amount_of_nights) {
				...
				// Nothing to do here...
			} elseif ($prearrival && $amount_of_nights + 1 == $old_amount_of_nights) {
				// User had a prearrival but no postdepature and all other nights
				unset($new_accos[$new_amount_of_nights - 1]);
			} elseif ($postdeparture && $amount_of_nights + 1 == $old_amount_of_nights) {
				// User had a postdeparture but no prearrival and all other nights
				unset($new_accos[0]);
			} elseif (!$prearrival && !$postdeparture && $amount_of_nights + 2 == $old_amount_of_nights) {
	...
	{
		return array_diff($this->getMembers(), $this->getParticipants(), $this->getAdmins(), $this->getTrainers());
	}
	...
	*/
	...
	public function buildUVGList($a_send, $a_filename)
	{
		...
		$row = $this->buildListMeta($workbook, $worksheet, "Trainingsteilnahmen", $this->gLng->txt("gev_excel_member_row_title"), $columns, $a_type);
	...
	public function buildMemberList($a_send, $a_filename, $a_type)
	{
		...
		$worksheet->setColumn(0, 0, 10);	// firstname
		$worksheet->setColumn(1, 1, 12); 	// lastname
		$worksheet->setColumn(2, 2, 12);	// org-unit
		...
		if ($a_type == self::MEMBERLIST_HOTEL) {
			...
			$worksheet->setColumn(3, 3, 25);	// Selbstzahler
			$worksheet->setColumn(4, 4, 25);	// status
			$worksheet->setColumn(5, 5, 20);	// overnights
			$worksheet->setColumn(6, 6, 20);	// pre_arrival
		...
		} elseif ($a_type == self::MEMBERLIST_PARTICIPANT) {
			$columns[] = "Funktion";
		...
		$row = $this->buildListMeta($workbook, $worksheet, $this->gLng->txt("gev_excel_member_title")." ".
										...
										), $this->gLng->txt("gev_excel_member_row_title"), $columns, $a_type);
		...
		if ($user_ids) {
			foreach ($user_ids as $user_id) {
				...
				$worksheet->writeString($row, 0, $user_utils->getFirstname(), $format_wrap);
				$worksheet->writeString($row, 1, $user_utils->getLastname(), $format_wrap);
				$worksheet->writeString($row, 2, $ou_title, $format_wrap);
				if ($a_type == self::MEMBERLIST_HOTEL) {
					$on_det = $user_utils->getOvernightDetailsForCourse($this->getCourse());
					usort($on_det, function ($d_a, $d_b) {
	//presorting user overnights makes table filling easier/more secure
					...
					$worksheet->write($row, 3, $user_utils->paysPrearrival() ? "Ja" : "Nein", $format_wrap);
					...
					$worksheet->write($row, 5, $on_fmla, $format_wrap);
					...
					while (ilDate::_before($day_iterator, $post_day)) {
						...
						$worksheet->write($row, 5 + $count_days, $has_overnight, $format_wrap);
						$day_iterator = new ilDate($day_iterator->increment(ilDateTime::DAY, 1), IL_CAL_UNIX);
						$count_days++;
					}
				} elseif ($a_type == self::MEMBERLIST_PARTICIPANT) {
					$worksheet->write($row, 3, $user_utils->getFunctionAtCourse($this->crs_id), $format_wrap);
				} else {
					$status = $user_utils->getAllIDHGBAADStatus();
					sort($status, SORT_STRING);
					$worksheet->write($row, 3, implode(", ", $status), $format_wrap);
					$worksheet->write($row, 4, " ".$user_utils->getMobilePhone(), $format_wrap);
					$worksheet->write($row, 5, " ", $format_wrap);
				}
			}
		}
	...
	*/
	...
	public static $hist_edu_programs = null;

	public static function getEduProgramsFromHisto()
	{
	...
	public static $hist_course_types = null;

	public static function getLearningTypesFromHisto()
	{
	...
	public static $hist_course_template_title = null;

	public static function getTemplateTitleFromHisto()
	{
	...
	public static $hist_participation_status = null;

	public static function getParticipationStatusFromHisto()
	{
	...
	public static function grantPermissionsForAllCoursesBelow($a_ref_id, $a_role_name, $a_permissions)
	{
		require_once("Services/GEV/Utils/classes/class.gevObjectUtils.php");
	...
	public static function revokePermissionsForAllCoursesBelow($a_ref_id, $a_role_name, $a_permissions)
	{
		require_once("Services/GEV/Utils/classes/class.gevObjectUtils.php");
	...
	public static function getAllCoursesBelow($a_ref_ids)
	{
		global $ilDB;
	...
	public static function timeWithinCourse(
		$timestamp,
		$ct_minutes,
		ilDateTime $a_start_date,
		ilDateTime $a_end_date,
	...
	public static function updateGDVTopic($gdv_topic, $a_ref_id)
	{
		require_once("Services/GEV/Utils/classes/class.gevObjectUtils.php");
		require_once("Services/GEV/Utils/classes/class.gevAMDUtils.php");
		require_once("Services/GEV/Utils/classes/class.gevSettings.php");
	...
	public static function updateTrainingCategory(array $categories, $a_ref_id)
	{
		require_once("Services/GEV/Utils/classes/class.gevObjectUtils.php");
		require_once("Services/GEV/Utils/classes/class.gevAMDUtils.php");
		require_once("Services/GEV/Utils/classes/class.gevSettings.php");
	...
	public static function updateTargetAndBenefits($targets, $a_ref_id)
	{
		require_once("Services/GEV/Utils/classes/class.gevObjectUtils.php");
		require_once("Services/GEV/Utils/classes/class.gevAMDUtils.php");
		require_once("Services/GEV/Utils/classes/class.gevSettings.php");
	...
	public static function updateContent($content, $a_ref_id)
	{
		require_once("Services/GEV/Utils/classes/class.gevObjectUtils.php");
		require_once("Services/GEV/Utils/classes/class.gevAMDUtils.php");
		require_once("Services/GEV/Utils/classes/class.gevSettings.php");
	...
	public static function updateWP($a_wp, $a_ref_id)
	{
		require_once("Services/GEV/Utils/classes/class.gevObjectUtils.php");
		require_once("Services/GEV/Utils/classes/class.gevAMDUtils.php");
		require_once("Services/GEV/Utils/classes/class.gevSettings.php");
	...
	*/
	...
	public function objsInCourseOfType($type = '')
	{
		return $this->gTree->getSubTree(
			$this->gTree->getNodeData($this->getRefId()),
			true,
			$type
		);
	}
