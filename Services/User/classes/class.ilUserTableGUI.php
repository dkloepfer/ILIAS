...
//		$this->setTitle($this->lng->txt("users"));
		...
		//gev-patch start #3021
		switch ($this->getMode()) {
			case self::MODE_LOCAL_USER:
				break;
			case self::MODE_USER_FOLDER:
				...
				//Need to set select all checkbox for both
			default:
...
//if ($GLOBALS["kk"]++ == 1) nj();
	...
	*/
	...
	protected function fillRow($user)
	{
		...
		if ($user["usr_id"] != 6) {
			//gev-patch start #3021 hide multicommand in local user gui
			if ($this->getMode() == self::MODE_USER_FOLDER or ($user['time_limit_owner'] == $this->getUserFolderId() && $this->getMode() !== self::MODE_LOCAL_USER)) {
			//gev-patch end
